...
genai.configure(api_key="AIzaSyCXydZTMFmdglWNEXgjVrX-nOIOtqmfMP8")

#function for predicting img 
def model_predict(image_path):
    model = tf.keras.models.load_model('plant_disease_cnn_model.keras')
    img = cv2.imread(image_path)
    Height, Width, Color = 224, 224, 3 #rgb
    img = cv2.resize(img, (Height, Width))
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    img = np.array(img)
    img = img.astype('float32')
    img = img / 255.0
    img = img.reshape(1, Height, Width, Color)
